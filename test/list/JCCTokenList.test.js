...
contract('TokenList', (accounts) => {
  ...
  it('TokenList test', async () => {
    ...
    // crossList = await al.getCrossList('0x0');
  ...
  it('TokenList test with different origin for token cross chain', async () => {
    ...
    console.log(tokens.JJCC.origin);
    ...
    crossList = await al.getCrossList(tokens.JJCC.origin);
    assert.equal(crossList.length, 4);
    ...
    crossList = await al.getCrossList(tokens.JJCC.origin);
    assert.equal(crossList.length, 3);
  });
});
