...
contract ERC20Factory is Administrative, IExtendedERC20, Payload {
  ...
  //constructor
  constructor(string name, string symbol, uint8 decimals, uint256 totalSupply, bool minable) Administrative() public {
    _name = name;
    _symbol = symbol;
    _decimals = decimals;
  ...
  function approve(address _spender, uint256 _value) public onlyPayloadSize(2 * 32) returns (bool) {
    // 检查设置数量和允许数量，防止重入修改
    require((_value == 0) || (_allowed[msg.sender][_spender] == 0), "check value not equal zero");
  ...
  function increaseAllowance(address _spender, uint256 _addedValue) public onlyPayloadSize(2 * 32) returns (bool) {
  ...
  function decreaseAllowance(address _spender, uint256 _subtractedValue) public onlyPayloadSize(2 * 32) returns (bool) {
    require(!_spender.isZeroAddress(), "can not send to zero account");
  ...
  function mint(address account, uint256 value) public onlyPrivileged onlyPayloadSize(2 * 32) returns (bool) {
  ...
  function burn(address account, uint256 value) public onlyPrivileged onlyPayloadSize(2 * 32) returns (bool) {
    require(!account.isZeroAddress(), "can not send to zero account");
    require(_minable, "minable must be true");
